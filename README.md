...
* 每个网格单元预测多个边界框, 每个目标只需要一个边界框预测器来负责
* Ii表示目标是否出现在网格单元i中，Iij表示网格单元i中的第j个边界框预测器“负责”该预测


### 主要创新点
* end-to-end的实时通用目标检测系统
* 隐式编码了上下文信息: 背景误检的概率比Fast-RCNN低，可以提升Fast-RCNN
* DackNet24网络架构
...
### 优缺点
* 优点:
    * 速度快: tiny YOLO超过150fps
    * 隐式编码了上下文信息: 背景误检的概率比Fast-RCNN低
    * 高泛化能力(相对于其他基于ＣＮＮ的滑动窗口或候选区域方案)
* 缺点:
    * 检出率比较相对较低，特别是小目标。
    * 强空间约束: 每个网格单元只预测两个盒子，只能有一个类别
    * 泛化能力弱(相对于DPM等): 数据驱动方法
    * 没有区别对待小边界框与大边界框的误差
