...
from dataloader.utils import meshgrid
...
class RetinaBoxCoder(object):
    def __init__(self, imgSize=640):
        ...
        self.anchor_boxes = self._get_anchor_boxes(imgSize=torch.FloatTensor([self.imgW, self.imgH]))  # [x,y,x,y]
    ...
    def _get_anchor_boxes(self, imgSize):
        '''Compute anchor boxes for each feature map.

        Args:
          imgSize: (tensor) model input size of (w,h).
        ...
        fm_sizes = [(imgSize/pow(2.0, i+3)).ceil() for i in range(num_fms)]  # p3 -> p7 feature map sizes

        boxes = []
        for i in range(num_fms):
            fm_size = fm_sizes[i]
            grid_size = imgSize / fm_size
            fm_w, fm_h = int(fm_size[0]), int(fm_size[1])
            xy = meshgrid(fm_w, fm_h) + 0.5  # [fm_h*fm_w, 2]
            xy = (xy*grid_size).view(fm_h, fm_w, 1, 2).expand(fm_h, fm_w, 9, 2)
            wh = anchor_wh[i].view(1,1,9,2).expand(fm_h,fm_w,9,2)
            box = torch.cat([xy-wh/2.0, xy+wh/2.0], 3)  # [x,y,x,y]
    ...
    def encode(self, boxes, labels):
        ...
        max_ious, max_ids = ious.max(1)  # 每个默认窗口最大iou对应的boxes
        boxes = boxes[max_ids]
